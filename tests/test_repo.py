...
# --------------------------------------------------------------------------------------------
# wiring + fakes  (the repo-folder builder is the shared ``make_repo`` conftest fixture)
# --------------------------------------------------------------------------------------------


@pytest.fixture
def fake_session(seed_page):
    """Stand-in for one agent session (installed via ``fake_agent(side_effect=fake_session)``,
    which records the calls). For a normal/repo ingest it writes a page citing ``rel_key`` (and,
    for a repo, a ``type: System`` page so the new category is exercised). For a ``delete``
...
def test_list_files_fallback_skips_junk(tmp_path, make_repo):
    root = make_repo(
        tmp_path,
        "r",
...
def test_build_digest_includes_header_and_listing(tmp_path, make_repo):
    root = make_repo(tmp_path, "svc", {"README.md": "# Svc\n", "app.py": "print(1)\n"})
...
def test_build_digest_truncates_long_file(tmp_path, make_repo):
    root = make_repo(tmp_path, "big", {"huge.py": "a" * 5000})
    digest = repo.build_digest(root, "raw/big", per_file_chars=100)
    assert "... [truncated]" in digest


def test_build_digest_respects_budget_and_prioritizes_signal(tmp_path, make_repo):
    root = make_repo(tmp_path, "p", {"transform.py": "T" * 400, "notes.txt": "N" * 400})
...
def test_build_digest_never_exceeds_budget(tmp_path, make_repo):
    ...
    root = make_repo(tmp_path, "big", files)
...
def test_build_digest_first_file_block_is_bounded(tmp_path, make_repo):
    # A single file far larger than the budget must not slip through whole — the cap holds.
    root = make_repo(tmp_path, "one", {"transform.py": "Z" * 50000})
    digest = repo.build_digest(root, "raw/one", max_chars=1500, per_file_chars=40000)
    assert len(digest) <= 1500


def test_build_digest_only_restricts_to_changed(tmp_path, make_repo):
    root = make_repo(tmp_path, "c", {"a.py": "A\n", "b.py": "B\n"})
...
def test_identity_snapshot_changes_with_content(tmp_path, make_repo):
    root = make_repo(tmp_path, "s", {"a.py": "one\n"})
...
def test_repo_ingested_as_one_source_not_per_file(repo_wiki, fake_agent, fake_session, make_repo):
    agent = fake_agent(side_effect=fake_session)
    raw = repo_wiki.raw
...
def test_repo_manifest_entry_is_commit_keyed(repo_wiki, fake_agent, fake_session, make_repo):
    agent = fake_agent(side_effect=fake_session)
...
def test_repo_creates_system_category_page(repo_wiki, fake_agent, fake_session, make_repo):
    fake_agent(side_effect=fake_session)
    make_repo(repo_wiki.raw, "svc", {"README.md": "# Svc\n", "app.py": "x\n"})

    ingest.ingest()
    assert (repo_wiki.wiki / "systems" / "acme-db.md").exists()
...
def test_repo_reingested_on_change(repo_wiki, fake_agent, fake_session, make_repo):
    agent = fake_agent(side_effect=fake_session)
    root = make_repo(repo_wiki.raw, "svc", {"README.md": "# Svc\n", "app.py": "x\n"})
    ...
    assert "raw/svc" in report.processed
    assert ("raw/svc", "repo-reconcile") in agent.calls
    after = manifest.entry_commit(manifest.load()["raw/svc"])
    assert before != after


def test_repo_deletion_reconciles_citations(repo_wiki, fake_agent, fake_session, make_repo):
    fake_agent(side_effect=fake_session)
    root = make_repo(repo_wiki.raw, "svc", {"README.md": "# Svc\n", "app.py": "x\n"})
...
def test_repo_support_off_falls_back_to_per_file(repo_wiki, fake_agent, fake_session, make_repo, monkeypatch):
