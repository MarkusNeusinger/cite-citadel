...
def test_ingest_guidance_is_scoped_to_the_call(tmp_citadel, monkeypatch):
    ...
    run fails (the server is long-lived — one caller's steer must never leak into the next), and
    an over-cap steer is refused as an error string before any session is spawned."""
    ...
    assert server.wiki_ingest(guidance="create one machine registry") == "error: stop here"
    assert seen == ["create one machine registry"]
    assert config.INGEST_GUIDANCE == ""  # restored after the call
    out = server.wiki_ingest(guidance="x" * (config.GUIDANCE_MAX_CHARS + 1))
    assert out.startswith("error: guidance is")
    assert len(seen) == 1  # the over-cap call never reached ingest
