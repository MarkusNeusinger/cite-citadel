...
Ten synthetic corpora live under [`corpora/`](https://github.com/MarkusNeusinger/cite-citadel/tree/main/corpora),
...
- **[werkhof](https://github.com/MarkusNeusinger/cite-citadel/tree/main/corpora/werkhof)** — uniform enumerable sources (a 28-machine register, a 20-code fault catalogue, a 15-customer CSV, plus a service report): `Registry`-page completeness (no entry compressed away), per-row citations with locators, promotion past the granularity floor, a dated status supersession, and near-miss key discipline — all fictional (Werkhof Anlagenservice Brandt).
...
injection-resistance, Clk = clockwork, Flu = flurfunk, Gaz = gazette, Kon = kontor, Wer = werkhof.)

| capability | Bev | Kel | Leu | Pem | Inj | Clk | Flu | Gaz | Kon | Wer |
| --- | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: | :-: |
...
| Registry completeness (enumerable sources, one cited row per entry) | | | | | | | | | | ● |
...
³ From the werkhof verify-corpus runs (2026-07-29 first run, 2026-08-06 regression): all three
collections came out as `type: Registry` pages with **28/28 machines, 20/20 fault codes, 15/15
customers**, one `[^sN]` per row with verifiable locators, the two promotions and the dated HX-201
supersession all in place — every guarantee held on both runs.
