...
extraction, visual image reading, a built-in PDF text-layer pre-pass (PDF citations get real,
offline-verifiable `lines A-B` locators — no extra install), opt-in whisper transcription for
audio/video recordings, multi-pass folding for oversized files, duplicate-format dedup, and a
