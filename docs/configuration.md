...
| `CITADEL_PDF_TEXT` | `auto` | The optional pypdf text-layer pre-pass: PDFs ingest via an extracted, content-addressed cached text layer (`.citadel_pdftext/` next to the wiki), giving real `lines A-B` locators that `lint`/`wiki_raw`/the viewer verify offline — and letting large PDFs chunk. `auto` uses it exactly when [pypdf](https://pypi.org/project/pypdf/) is importable (`pip install cite-citadel[pdf]`); `1` forces it (per-source fallback + `doctor` WARN when pypdf is missing); `0` forces agent-native PDF reading (`p. N` locators, agent-verified). Scanned/encrypted/unparsable PDFs always fall back to agent-native reading. |
