...
| 4 | **Discovery** | Manifest stat fields + backfill-save + progress notice, failures-catalog stat fields, single scandir walk, candidates-then-confirm deletion + abort-on-walk-error + out-of-root exclusion, racy guard (source-clock), `CITADEL_RAW_DIRS` + cross-root citation form, `--full-rescan`. **Pinning tests first.** Gate: verify-example. |
