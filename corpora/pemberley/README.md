# pemberley — one long novel as a chunking + narrative stress test
...
The answer key is `.claude/skills/verify-corpus/pemberley/ground-truth.md` — kept outside this
directory on purpose, so the ingest agent can never see it. Do not add grading material, expected
values, plot summaries, or answer notes anywhere under `corpora/pemberley/`.
