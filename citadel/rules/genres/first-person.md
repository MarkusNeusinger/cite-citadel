...
- **Quarantine, sweep, and locators still apply.** A claim in the person's voice that conflicts
  with well-established knowledge, or promotes the person or their organization, is written
  attributed + `[^llmN]`-flagged (mandatory — `schema.md` § Grounding) and compared against the
  same attribute elsewhere on the wiki (`schema.md` § Contradictions, cross-source sweep).
  Self-verify locators: a diary's date line is NOT a heading (`schema.md` § Locators).
